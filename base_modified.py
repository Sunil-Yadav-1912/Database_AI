...
class VannaBase(ABC):
    ...
    def log_to_s3(self, question, response):

        # AWS S3 Configuration
        AWS_REGION = configures.AWS_REGION
        S3_BUCKET_NAME = configures.S3_BUCKET_NAME
        S3_FOLDER_PATH = configures.S3_FOLDER_PATH

        # Initializing S3 client
        s3 = boto3.client(
            "s3",
            aws_access_key_id=configures.AWS_ACCESS_KEY,
            aws_secret_access_key=configures.AWS_SECRET_KEY,
            region_name=AWS_REGION
        )

        today = datetime.utcnow().strftime("%Y-%m-%d")
        s3_file_key = f"{S3_FOLDER_PATH}llm_logs_{today}.json"
        ...
        try:
            print(f"Uploading logs to S3: {s3_file_key}")
            s3.put_object(
            ...
            print(f"Successfully logged to {s3_file_key}")
        except botocore.exceptions.ClientError as e:
    ...
    def generate_sql(self, question: str, allow_llm_to_see_data=False, **kwargs) -> str:
        ...
        self.log_to_s3(question=question, response=llm_response)
    ...
    def get_sql_prompt(
        ...
        **kwargs,
        ...
        if initial_prompt is None:
            ...
            "Please help to generate a SQL query to answer this question without any explanation. Your response should ONLY be based on the given context and follow the response guidelines and format instructions. "
